        ...
        <div className="clearfix">
			...
			<ul className="pagination">
                ...
                {numOfPages.map((pages, index) => {
                    return (
                        <li key={index} className={`${currentButton === pages ? 'page-item activ' : 'page-item' } `}>
                            <a href="#!" className="page-link" onClick={() => setCurrentButton(pages)}>
                                {pages}
                            </a>
                        </li>
                ...
                <li className={`${currentButton === numOfPages.length ? 'page-item disabled' : 'page-item' } `}>
                    <a href="#!" onClick = { () => setCurrentButton((next) => next === 1 ? next : next + 1)}>Next</a>
                </li>
			</ul>
		</div>
    )
